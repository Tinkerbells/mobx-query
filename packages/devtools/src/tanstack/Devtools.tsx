...
const DraggablePanel: Component<DevtoolsPanelProps> = (props) => {
  ...
  createEffect(() => {
    ...
    rootContainer.style[styleProp] = `${isVertical ? props.localStore.width : props.localStore.height
      }px`
...
export const ContentView: Component<ContentViewProps> = (props) => {
  ...
  // mobx-query НЕ кеширует mutations, поэтому показываем только queries
  ...
  const queries = createMemo(
    on(
      ...
      () => {
        ...
        // mobx-query НЕ имеет disabled queries, фильтр закомментирован
    ...
    on(
      ...
      () => {
        ...
        const filtered = props.localStore.mutationFilter
          ? curr.filter((item) => {
            const value = `${item.options.mutationKey
        ...
        <div class={cx(styles().row, 'tsqd-header')}>
          <div class={styles().logoAndToggleContainer}>
            ...
            <RadioGroup.Root
              ...
              value={selectedView()}
              onChange={(value) => {
                setSelectedView(value as 'queries')
                setSelectedQueryHash(null)
            ...
            >
              <RadioGroup.Item value="queries" class="tsqd-radio-toggle">
                ...
                {/* mobx-query НЕ кеширует mutations, вкладка скрыта т.к и так только queries отображаются */}
                {/* <RadioGroup.ItemLabel title="Toggle Queries View"> */}
                {/*   Queries */}
                {/* </RadioGroup.ItemLabel> */}
              </RadioGroup.Item>
              {/* mobx-query НЕ кеширует mutations, вкладка скрыта */}
        ...
        <div class={cx(styles().row, 'tsqd-filters-actions-container')}>
          <div class={cx(styles().filtersContainer, 'tsqd-filters-container')}>
            ...
            <button
              ...
              aria-label={`Sort order ${(selectedView() === 'queries'
                ? sortOrder()
                : mutationSortOrder()) === -1
                ? 'descending'
                : 'ascending'
            ...
            <DropdownMenu.Root gutter={4}>
              ...
              >
                ...
                >
                  ...
                  {/* mobx-query НЕ имеет disabled queries, настройка закомментирована */}
                  {/* <DropdownMenu.Sub overlap gutter={8} shift={-4}>
                    <DropdownMenu.SubTrigger
                      class={cx(
                        styles().settingsSubTrigger,
...
const QueryRow: Component<{ query: Query }> = (props) => {
  ...
  // mobx-query НЕ имеет disabled и static queries
    ...
    (queryCache) =>
      queryCache()
        ...
        .filter((q) => {
          ...
          return label === 'inactive' || label === 'idle'
        }).length,
  )
...
const QueryDetails = () => {
  ...
  // mobx-query специфичный статус - background loading
  ...
  // mobx-query InfiniteQuery специфичный статус - конец списка достигнут
    ...
    <Show when={activeQuery() && activeQueryState()}>
      ...
      >
        ...
        >
          ...
          {/* mobx-query НЕ хранит dataUpdatedAt */}
